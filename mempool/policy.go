...
const (
	...
	DefaultMinRelayTxFee = dogeutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee dogeutil.Amount) int64 {
	...
	if minFee < 0 || minFee > dogeutil.MaxSatoshi {
		minFee = dogeutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *dogeutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee dogeutil.Amount) bool {
...
func checkTransactionStandard(tx *dogeutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee dogeutil.Amount,
...
func GetTxVirtualSize(tx *dogeutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
