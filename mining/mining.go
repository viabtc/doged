...
const (
	...
	MinHighPriority = dogeutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr dogeutil.Address) (*dogeutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *dogeutil.Tx, height int32) error {
...
func logSkippedDeps(tx *dogeutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress dogeutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*dogeutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := dogeutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := dogeutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A dogeutil.Block should use saved in the state to avoid
