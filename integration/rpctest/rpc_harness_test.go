...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt dogeutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(dogeutil.Amount(5 * dogeutil.SatoshiPerBitcoin))
	...
	txid = genSpend(dogeutil.Amount(500 * dogeutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(dogeutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*dogeutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, dogeutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := dogeutil.Amount(250 * dogeutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = dogeutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := dogeutil.Amount(50 * dogeutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := dogeutil.Amount(numMatureOutputs * 50 * dogeutil.SatoshiPerBitcoin)
