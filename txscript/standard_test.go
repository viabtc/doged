...
// newAddressPubKey returns a new dogeutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) dogeutil.Address {
	addr, err := dogeutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new dogeutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) dogeutil.Address {
	addr, err := dogeutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new dogeutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) dogeutil.Address {
	addr, err := dogeutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the dogeutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the dogeutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := dogeutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := dogeutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := dogeutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := dogeutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := dogeutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*dogeutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*dogeutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*dogeutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*dogeutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*dogeutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
