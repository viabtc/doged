...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: dogeutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := dogeutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*dogeutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, dogeutil.NewTx(tx))
	}
...
func additionalCoinbase(amount dogeutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee dogeutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee dogeutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee dogeutil.Amount) *wire.MsgTx {
